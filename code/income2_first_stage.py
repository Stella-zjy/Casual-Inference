...
from income2_data_processor import get_stage1_input
...
def build(x, probilities, times=1, save=False):
    ...
    if save:
        new.to_csv("../data/income_data2/sample_data.csv")
...
def solve_stage_one(ml_method="LR", save=False):
    x, y = get_stage1_input()
    ret = x.groupby(by=['sob'])
    d = {
    ...
    new_x = build(x, probilities, 100, save)
